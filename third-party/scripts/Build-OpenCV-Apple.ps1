...
ForEach($BuildTypeFromList In $BuildTypes) {
    ForEach($Platform In $PlatformArchitectureCombinations.Keys) {
        ...
        ForEach($Architecture In $Architectures) {
            & "$PSScriptRoot/Build-OpenCV-Library.ps1" $Platform $Architecture $BuildTypeFromList
            If($LastExitCode -Ne 0) {
                Write-Error -Message "Failed to build OpenCV library for '$Platform' platform, '$Architecture' architecture and '$BuildTypeFromList' build type."
                Exit 1
            }
        }
    }
