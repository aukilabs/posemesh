...
#include <Posemesh/C/ArucoDetection.h>
#include <cassert>
#include <cstring>
#include <limits>
#include <memory>
...
{
    if (!out_contents) {
        assert(!"psm_aruco_detection_detect_aruco(): out_contents is null");
        return false;
    }
    if (!out_corners) {
    ...
    if (contents.size() > std::numeric_limits<uint32_t>::max()) {
        assert(!"psm_aruco_detection_detect_aruco(): contents count overflow");
        return false;
    }
    ...
    *contents_prefix_ptr = nullptr;
    ...
    if (corners.size() > std::numeric_limits<uint32_t>::max()) {
        assert(!"psm_aruco_detection_detect_aruco(): corners count overflow");
        return false;
    }
